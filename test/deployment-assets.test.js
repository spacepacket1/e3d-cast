...
test('pm2 ecosystem defines pod2vid ui and worker processes', () => {
  const apps = Array.isArray(ecosystem.apps) ? ecosystem.apps : [];
  const names = apps.map((app) => app.name);

  assert.ok(names.includes('pod2vid-ui'));
  assert.ok(names.includes('pod2vid-worker'));

  const uiApp = apps.find((app) => app.name === 'pod2vid-ui');
  const workerApp = apps.find((app) => app.name === 'pod2vid-worker');

  assert.strictEqual(uiApp.script, 'src/server/index.js');
  assert.strictEqual(uiApp.env.POD2VID_UI_PORT, 3200);
  assert.strictEqual(uiApp.env.POD2VID_UI_DIR, 'dist');

  assert.strictEqual(workerApp.script, 'src/worker/index.js');
  assert.strictEqual(workerApp.instances, 1);
  assert.strictEqual(workerApp.env.POD2VID_JOB_RUNNER, '/home/ubuntu/e3d-pod2vid/bin/pod2vid-job.py');
});

test('nginx deployment config enforces tls, upload limits, and required proxy routes', () => {
  const configPath = path.join(repoRoot, 'deploy', 'nginx', 'pod2vid.e3d.ai.conf');
  ...
  assert.match(config, /ssl_certificate \/etc\/letsencrypt\/live\/pod2vid\.e3d\.ai\/fullchain\.pem;/);
  assert.match(config, /location \/api\/pod2vid\/\s*\{[\s\S]*proxy_pass http:\/\/127\.0\.0\.1:3000;/);
...
test('deployment documentation includes production verification and rollback instructions', () => {
  ...
  assert.match(docs, /\/api\/pod2vid\/health/);
  assert.match(docs, /Get E3D/);
  assert.match(docs, /Rollback/);
  assert.match(docs, /pm2 reload ecosystem\.config\.js --only pod2vid-ui,pod2vid-worker/);
